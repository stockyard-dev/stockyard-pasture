...
async function load(){var r=await fetch(A+'/posts').then(function(r){return r.json()});items=r.posts||[];document.getElementById('stats').innerHTML='<div class="st"><div class="st-v">'+items.length+'</div><div class="st-l">Total</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div>';render();}
function render(){var q=(document.getElementById('search').value||'').toLowerCase();var f=items;
if(q)f=f.filter(function(i){return (i.title||'').toLowerCase().includes(q)||(i.body||'').toLowerCase().includes(q)||(i.platform||'').toLowerCase().includes(q)||(i.status||'').toLowerCase().includes(q)});
...
async function del(id){if(!confirm('Delete?'))return;await fetch(A+'/posts/'+id,{method:'DELETE'});load();}
function formHTML(item){var i=item||{title:"",body:"",platform:"",status:"",scheduled_at:"",published_at:"",tags:"",total:"",draft:"",scheduled:"",published:""};var isEdit=!!item;
var h='<h2>'+(isEdit?'EDIT':'NEW')+' PASTURE</h2>';
...
h+='<div class="fr"><label>Draft</label><input id="f-draft" value="'+esc(i.draft)+'"></div>';
h+='<div class="fr"><label>Scheduled</label><input id="f-scheduled" value="'+esc(i.scheduled)+'"></div>';
h+='<div class="fr"><label>Published</label><input id="f-published" value="'+esc(i.published)+'"></div>';
...
async function submit(){var body={title:document.getElementById('f-title').value.trim(),body:document.getElementById('f-body').value.trim(),platform:document.getElementById('f-platform').value.trim(),status:document.getElementById('f-status').value.trim(),scheduled_at:document.getElementById('f-scheduled-at').value.trim(),published_at:document.getElementById('f-published-at').value.trim(),tags:document.getElementById('f-tags').value.trim(),total:document.getElementById('f-total').value.trim(),draft:document.getElementById('f-draft').value.trim(),scheduled:document.getElementById('f-scheduled').value.trim(),published:document.getElementById('f-published').value.trim()};
if(editId){await fetch(A+'/posts/'+editId,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}
else{await fetch(A+'/posts',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}closeModal();load();}
